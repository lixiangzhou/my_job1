//
//  PatientCommonScaleNormalCell.swift
//  doctor
//
...
class PatientCommonScaleNormalCell: UITableViewCell {
...
// MARK: - UI
extension PatientCommonScaleNormalCell {
    private func setUI() {
        addLeftView()
    ...
    func addLeftView() {
        ...
        leftTopLine.snp.makeConstraints { (make) in
            ...
            make.bottom.equalTo(leftCircleView.snp.top)
        }
    ...
    func addRightView() {
        rightView = RoundOptionView(frame: CGRect(x: 106, y: 8, width: UIScreen.zz_width - leftW - 16, height: 224), radius: 4, roundColor: .cF5F8FF, bgColor: .white, roundingCorners: .allCorners)
        ...
        improveLabel = addRowView(idx: 5, title: "治疗改善率")
        standardLabel = addRowView(idx: 6, title: "达标标准")
        resultLabel = addRowView(idx: 7, title: "达标结果")
        
        rightView.snp.makeConstraints { (make) in
            make.top.equalTo(8)
            make.left.equalTo(leftW)
            make.right.equalTo(-16)
            make.height.equalTo(rightView.zz_height)
