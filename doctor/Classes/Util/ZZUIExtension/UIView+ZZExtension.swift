...
public extension UIView {
    ...
    func zz_addCircleBorder(color: UIColor, width: CGFloat, radius: CGFloat) {
        let frame = CGRect(x: -0.5, y: -0.5, width: radius * 2 + 1, height: radius * 2 + 1)
        ...
        layer.cornerRadius = (frame.width - frame.origin.x * 2) * 0.5
...
public extension UIView {
    ...
    /// - returns: 添加的子控件
    @discardableResult
    func zz_add(subview: UIView, frame: CGRect? = nil, bgColor: UIColor? = nil) -> UIView {
        addSubview(subview)
        if let rect = frame {
            subview.frame = rect
        }
    ...
    /// 添加子控件
    ///
    /// - parameter subviews: 子控件
    func zz_add(subviews: [UIView]) {
        for sub in subviews {
            addSubview(sub)
        }
    }
